...
bt_start = {
        "inline_keyboard": [
                ...
                [{"text": f"💳 Comprar CC", "callback_data": "comprar"},
                {"text": f"💳 Consultaveis", "callback_data": "lista_consultaveis"}],
                [{"text": f"💠 Adicionar Saldo", "callback_data": "comprar_saldo"},
                {"text": f"💼Minha carteira","callback_data": "carteira"}],
                [{"text": f"⚖️ Troca CC", "callback_data": "reembolso"}],
                [{"text": f"🏆 Ranking", "callback_data": "ranking"}],
                [{"text": f"👾 Criador Do bot", "url": "https://t.me/C4R7M4N"},
                {"text": f"🧑‍💻 Suporte", "url": f"https://t.me/{suporte}"}],
...
def bt_dono():
    bt_dono = {
            "inline_keyboard": [
                ...
                {"text": f"💳 CONSULTAVEL", "callback_data": "lista_consultaveis"}],
                [{"text": f"COMPRAR SALDO", "callback_data": "comprar_saldo"},
                {"text": f"💼 CARTEIRA", "callback_data": "carteira"}],
                [{"text": f"⚖️ TROCA INFO", "callback_data": "reembolso"}],
                [{"text": f"✪ Ranking", "callback_data": "ranking"}],
                [{"text": f"🧠 Criador do bot", "url": "https://t.me/Rtzinmaker"},
                {"text": f"🧑‍💻 Suporte", "url": f"https://t.me/{suporte}"}],
                [{"text": f"Painel Rtzinmaker 🛰️", "callback_data": "admin"}],
                ]
            }
                        ...
                        if consul['consul'] in cc_consultadas:
                            try:
                                texto = (
                                    ...
                                    f"      🚻CPF `{consul['cpf']}`\n\n"
                                    f"      ☎️Telefone `{consul['telefone']}`\n\n"
                                    
                                    f"      *PARA ACESSAR SUA PORTO SEGURO CLICA AKI* @linkporto\n\n"
                                    
                                    
                                    f"      *VENCIMENTO DA FATURA DIA 10*\n\n"

                                    
                                    f"*⚠️ \| TERMOS DE TROCA*\n\n"
                                    f"      ♻️ 10 Minutos para troca em caso de erro no acesso\n"
                                    f"      ♻️ 20 Minutos para troca por CVV ou VALIDADE errada, *mediante a gravação debitando na americanas*\n"
                                    f"      ♻️ Troca somente dentro dos prazos de troca\n"
                                )

                                bt = {
                                "inline_keyboard": [
                                        [{"text": f"🔄 Troca", "url": "https://t.me/C4R7M4N"}]
                                    ]
                                }

                                # DESCONTA O SALDO
                                cliente['saldo'] -= consul['preco']
                            ...
                            try:
                                ...
                                texto = (
                                    ...
                                    f"*⚠️ \| TERMOS DE TROCA*\n\n"
                                    f"      ♻️ 10 Minutos para troca em caso de erro no acesso\n"
                                    f"      ♻️ 01 Hora para troca por CVV ou VALIDADE errada, *mediante a gravação debitando na americanas*\n"
                                    f"      ♻️ Troca somente dentro dos prazos de troca\n"
                                )
...
async def funcao_ler_menssagens(message: types):
    ...
    if menssagem.isnumeric()  and tipo == 'private' and len(menssagem) == 12:
    ...
    # GERA GIFT
    if menssagem.isnumeric()  and tipo == 'private':
        if id == func_bot.id_dono or id in adm():
